...
router.get("/customer", (req, res) => {
  if (req.session.user) {
    if (req.session.radiobtn == "customer") {
      //if login as customer was chosen only customer profile dashboard will be accessible
    ...
    } else if (req.session.radiobtn == "clerk") {
      //if login as data clerk was chosen then redirects to that route
...
router.post("/customer", (req, res) => {
  ...
  if (hasMeals) {
    ...
    cart.forEach((mealkit) => {
      string.push(
        `<div><div> ${mealkit.qty} of ${mealkit.meal.name}</div> <div>$${mealkit.meal.price} each</div></div><br>`
      );
    });
    string = string.join("");
    ...
    const msg = {
      ...
      from: "mohammedfuhad1@yahoo.com",
      subject: "Order Summary",
      html: `Hey ${req.session.user.firstName} ${req.session.user.lastName},<br>
  ...
  if (req.session.user) {
    if (req.session.radiobtn == "clerk") {
      ...
      if (req.session.imgError) {
        errs.imgErr =
          "The file type is not supported please update the meal with a supported type.";
      ...
      .then((mealSaved) => {
        if (req.files) {
          ...
          if (
            ...
            console.log(
              `Meal ${mealSaved.name} has been saved to the database.`
            );
            req.files.imageUrl.name = `meal_pic_${mealSaved._id}${
              path.parse(req.files.imageUrl.name).ext
            }`;
            req.files.imageUrl
              .mv(`public/images/meals/${req.files.imageUrl.name}`)
              .then(() => {
                mealModel
                  .updateOne(
                    {
    ...
    .then(() => {
      ...
      if (req.files) {
        ...
        if (extention != ".PNG" && extention != ".GIF" && extention != ".JPG") {
          ...
          req.files.imageUrl.name = `meal_pic_${req.body._id}${
            path.parse(req.files.imageUrl.name).ext
          }`;
          req.files.imageUrl.mv(
            `public/images/meals/${req.files.imageUrl.name}`
          );
